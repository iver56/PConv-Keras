...
class PConvUnet(object):
    ...
    def build_pconv_unet(self, train_bn=True):
        ...
        def encoder_layer(img_in, mask_in, filters, kernel_size, bn=True):
            conv, mask = PConv2D(filters, kernel_size, strides=2, padding="same")(
                [img_in, mask_in]
            )
            if bn:
        ...
        # DECODER
        def decoder_layer(
            img_in, mask_in, e_conv, e_mask, filters, kernel_size, bn=True
        ):
    ...
    def gram_matrix(x):
        ...
        ), "Please use channels-last format"
    ...
    def perform_chunking(self, img_size, chunk_size):
        ...
        while True:
            chunks.append(
                (
                    i * (chunk_size - self.img_overlap / 2),
                    i * (chunk_size - self.img_overlap / 2) + chunk_size,
                )
            )
            i += 1
        ...
        n_count = len(chunks)
        chunks[-1] = tuple(
            x - (n_count * chunk_size - img_size - (n_count - 1) * self.img_overlap / 2)
            for x in chunks[-1]
        )
    ...
    def dimension_preprocess(self, img):
        ...
        if img.shape[0] < self.img_rows:
            padding = np.ones(
                (self.img_rows - img.shape[0], img.shape[1], img.shape[2])
            )
        ...
        if img.shape[1] < self.img_cols:
            padding = np.ones(
                (img.shape[0], self.img_cols - img.shape[1], img.shape[2])
            )
